...
5. **State Marker Protocol & Git Trailer Evidence:**
   The review loop maintains state via the assistant-authored `WAKEWIRE_REVIEW_STATE` marker in the conversation transcript and `WakeWire-Review-PR` / `WakeWire-Review-Round` git commit trailers. A round identifies an authoritative actionable finding set rather than a single commit, so a scoped validation or CI correction for that set may reuse the current round. A newly evaluated actionable review always advances the round. Reconciliation still requires an ordered, gap-free sequence whose highest round matches the marker. This ensures the 5-round remediation cap and execution state can be audited and recovered directly from Git history without requiring resets after same-pass corrections.
6. **App Server Capability Admission Preflight & M5 Status (2026-08-21):**
   - **Step 1 Capability Admission Evidence:** Verified on `codex-cli 0.149.0` and recorded at `/private/tmp/wakewire-codex-review-admission.md` (turn `01a026ad-6f06-7b50-a242-83c9323898b4`, target PR `https://github.com/bmorrison/wakewire/pull/4` on branch `disposable/codex-review-loop-exec`, commit `72c17cf`). The initial isolated probe used empty workspace roots and established App Server capability, but did **not** prove a managed resumed thread could write Git metadata. PR #3 live evidence found that `<cwd>/.git` is more-specific and read-only unless explicitly rooted; the completed route therefore uses only authoritative `thread/resume.cwd` plus `<cwd>/.git` and proves successful fetch, commit, push, and final clean delivery under that policy. The earlier disposable PR/branch has since been closed and deleted by the supervisor.
   - **Milestone 5 Live Execution Status:** Supervised execution completed on 2026-08-21/22 UTC against disposable `bmorrison/wakewire` PR #3, never merging. The shared loopback App Server and a plain attached `codex --remote` CLI processed exact signed PR/actor ingress, settle/coalescing, negative filters/dedup, replay isolation, and busy retry recovery. A deliberately injected real P1 was remediated in `c5c402337cc23c78fc7d38ff3b3759be3df84411` with the required trailers and one non-force push/re-review request; the final signed wake reported an exit-0 clean verdict for that same current head with no mutation. Durable daemon evidence is `/private/tmp/wakewire-m5-pr3/logs/wakewire.log`; the final target-thread transcript is `/Users/burkmorrison/.codex/sessions/2026/08/21/rollout-2026-08-21T19-03-08-01a02735-22de-7273-b02f-fda5907c8054.jsonl`.
7. **Authoritative Git metadata root for resumed workspace writes (M5 live finding, 2026-08-21):** `thread/resume.cwd` is the sole authority for a resumed workspace-write turn. The emitted App Server policy grants exactly that checkout and `<cwd>/.git`; the managed profile otherwise treats `.git` as a more-specific read-only path and blocks `git fetch` from updating `FETCH_HEAD`. The adapter fails closed when workspace-write resume omits `cwd`; it never accepts a route or prompt supplied directory. The live review topology therefore requires a standalone clone with a physical, writable `<cwd>/.git`, not a linked worktree whose Git metadata redirects outside the checkout. This is a narrow Git-metadata allowance, not an expansion to a parent directory or arbitrary paths.
